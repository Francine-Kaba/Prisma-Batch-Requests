...
class audit_log(BaseModel):
    ...
    @classmethod
    def prisma(cls) -> 'actions.audit_logActions':
        from .client import get_client

        return actions.audit_logActions(get_client(), cls)
    ...
    def __init_subclass__(
        ...
        **kwargs: Any,
        ...
        super().__init_subclass__()
        if warn_subclass:
            _maybe_warn_subclassing(cls.__name__, 'audit_log', stacklevel=3)

    @staticmethod
    def create_partial(
        name: str,
        include: Optional[Iterable['types.audit_logKeys']] = None,
        exclude: Optional[Iterable['types.audit_logKeys']] = None,
        required: Optional[Iterable['types.audit_logKeys']] = None,
        optional: Optional[Iterable['types.audit_logKeys']] = None,
        relations: Optional[Mapping['types.audit_logRelationalFieldKeys', str]] = None,
        exclude_relational_fields: bool = False,
    ) -> None:
        if not os.environ.get('PRISMA_GENERATOR_INVOCATION'):
            raise RuntimeError(
        ...
        fields: Dict['types.audit_logKeys', PartialModelField] = {}

        try:
            if include:
                for field in include:
                    fields[field] = _audit_log_fields[field]
            elif exclude:
                for field in exclude:
                    if field not in _audit_log_fields:
                        raise KeyError(field)

                fields = {
                    key: data
                    for key, data in _audit_log_fields.items()
                    if key not in exclude
                }
            else:
                fields = _audit_log_fields.copy()
            ...
            if relations:
                raise ValueError('Model: "audit_log" has no relational fields.')
        except KeyError as exc:
            raise ValueError(
                f'{exc.args[0]} is not a valid audit_log / {name} field.'
            ) from None
...
_audit_log_relational_fields: Set[str] = set()  # pyright: ignore[reportUnusedVariable]
_audit_log_fields: Dict['types.audit_logKeys', PartialModelField] = {
    'id': {
        'name': 'id',
        'is_list': False,
        'optional': False,
